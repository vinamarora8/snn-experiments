...
# filenames
stimuli_filename = "/home/ven0m/code/btp/simple-0/stimulus.npy"
''' network_filename = "/home/ven0m/code/btp/simple-0/network.brian2" '''

# ----------------------------
# Creating the neuron (output)
# ----------------------------
print("Creating neuron... ", end='', flush=True)
eqns = '''
...
G_op = b.NeuronGroup(1, eqns, method='exact',
...
stdp_post = 'Apost = Apost_reset; w += Apre*int(w < w_max - Apre) + w_max*int(w >= w_max - Apre)'
...
# ----
plt.figure("float1")
clist = ['b', 'orange', 'g', 'r', 'black']
for k in range(1):
    plt.plot(SM_neuron.t/b.second, SM_neuron.v[k], color=clist[k], label=str(k))
    plt.plot(SM_neuron.t/b.second, SM_neuron.v_thresh[k], color=clist[k], linestyle='--')
    plt.plot(SM_neuron.t/b.second, SM_neuron.inhibiting[k]*0.005, color=clist[k], linestyle=':')
    plt.plot(SM_synapse.t/b.second, SM_synapse.w[0], color='orange')
    plt.legend()
for k in range(len(M1.t)):
    plt.axvline(M1.t[k]/b.second, linestyle=':', color=clist[M1.i[k]])
plt.show()
